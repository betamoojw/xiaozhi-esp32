...
public:
    explicit FtpClientSession(int id, std::string_view local_address,
        ...
        : BaseComponent("FtpClientSession " + std::to_string(id)),
          ...
          root_directory_(root_path.lexically_normal()),
...
protected:
    ...
    bool handle_pwd(std::string_view arguments) {
        ...
        auto relative = current_directory_.lexically_relative(root_directory_);
        std::string virtual_path =
            relative.empty() || relative == "." ? "/" : "/" + relative.generic_string();
        std::string message = std::string{"\""} + virtual_path + "\" is the current directory";
        return send_response(257, message);
    }

    std::optional<std::filesystem::path> resolve_path(std::string_view value) const {
        std::filesystem::path requested(value);
        const bool absolute = requested.is_absolute();
        if (requested.is_absolute()) {
            requested = requested.relative_path();
        }
        const auto candidate =
            ((absolute ? root_directory_ : current_directory_) / requested).lexically_normal();
        auto root_iterator = root_directory_.begin();
        auto candidate_iterator = candidate.begin();
        for (; root_iterator != root_directory_.end(); ++root_iterator, ++candidate_iterator) {
    ...
    bool handle_cwd(std::string_view arguments) {
        ...
        auto new_directory = resolve_path(directory);
        if (!new_directory) {
            return send_response(550, "Path escapes the FTP root.");
        }
    ...
    bool handle_cdup(std::string_view arguments) {
        ...
        auto parent = resolve_path("..");
        if (!parent) {
            return send_response(550, "Already at the FTP root.");
        }
    ...
    bool handle_rmd(std::string_view arguments) {
        ...
        std::string_view path = arguments.substr(0, path_end);
        auto full_path = resolve_path(path);
        if (!full_path || *full_path == root_directory_) {
            return send_response(550, "Path escapes or names the FTP root.");
        }
        if (!std::filesystem::exists(*full_path)) {
            return send_response(550, "File does not exist.");
        }
    ...
    bool handle_mkd(std::string_view arguments) {
        ...
        std::error_code error;
        if (!std::filesystem::create_directory(*full_path, error)) {
            logger_.error("Failed to create directory: {}", error.message());
            return send_response(550,
                                 fmt::format("Failed to create directory: {}", error.message()));
        }
