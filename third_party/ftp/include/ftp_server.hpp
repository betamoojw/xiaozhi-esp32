...
protected:
    ...
    bool accept_task_function(std::mutex& m, std::condition_variable& cv, bool& task_notified) {
        ...
        logger_.info("Accepted connection from {}, id {}", client_ptr->get_remote_info(),
                     client_id);

        // create a new client session
        auto client_session_ptr = std::make_unique<FtpClientSession>(client_id, ip_address_,
                                                                     std::move(client_ptr), root_);
